...
export const EditUserDialog = ({ user, id, users }) => {
  ...
  useEffect(() => {
    const updateFirebaseDoc = async () => {
      ...
      if (submitPressed) {
        try {
        updateFirebaseDoc();
          toast.success(`Successfully edited ${userEmail}!`, {
            position: "top-right",
            autoClose: 5000,
          ...
          toast.error(`Error edited ${userEmail}!`, {
            position: "top-right",
            autoClose: 5000,
            hideProgressBar: false,
            closeOnClick: true,
  ...
  useEffect(() => {
    setUserEmail(user.email); // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [users]);
